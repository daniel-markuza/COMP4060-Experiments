...
#define CH_AD_INTERVAL_MS 1000UL  // Interval between CH advertisements (in ms)
...
static void runRound(void)
{
    ...
    while (getMsCount() - roundStart < ROUND_DURATION_MS && !roundEnded)
    {
        ...
        {
            sendMemberData();
            // Limit members to one DATA send per round.
            break;
        }
    }
...
{
    systemInitialize();
    delayMs(1000); // Allow hardware to initialize
